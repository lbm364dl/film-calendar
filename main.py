...
def run_archive(args):
    ...
    historical_films = read_master_json(args.output)  # empty list if file missing

    # Build historical indices: short_url -> index, and fallback (title, director, year) -> index
    ...
    for film in live_films:
        ...
        for session in sessions:
            ...
            try:
                ...
                # Can't parse — keep it in live DB
        ...
        if not in_range:
            # Film has no sessions in range — keep it untouched
        ...
        if hist_idx is not None:
            # Film already exists in historical — append new sessions only
            ...
            # New film for the historical DB — copy everything, set sessions to in_range
